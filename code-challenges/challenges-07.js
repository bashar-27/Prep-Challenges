...
const objLat = (obj) => {
  ...
  } ${lastName} I am ${age} YO, and I love ${hobby}.`;
};
// -------------------------------------------------------------------------------------------------------
...
const cvFormatter = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].yearsOfExperience > 1 && arr[i].lastName != null) {
      ...
      newFormat.push(updateData);
      //    newFormat.push(newFormat[i].fullName=arr[i].fullName);
      //    newFormat.push(newFormat[i].tech=arr[i].tech);
...
const classesAvg = (data) => {
  ...
  for(let i=0 ; i<data.grades.length; i++){
    ...
    for(let j=0 ; j<grade.numberOfClasses; j++){
        let numClass= grade.classes[j];
       return avg=Math.floor((numClass.classScores.reduce((a,b)=>a+b))/ numClass.classScores.length);
    }
   
  }
  console.log(data.avg = Math.floor(sum/data.classScores.length));
};
// -------------------------------------------------------------------------------------------------------
